...
from plugins.alist2strmpro.alist import AlistClient, AlistFile
...
class Alist2StrmPro(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "支持从多个alist目录生成strm。"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/JaylenMar/MoviePilot-Plugins/main/icons/Alist.png"
    # 插件版本
    plugin_version = "1.0"
    # 插件作者
    plugin_author = "JaylenMar"
    # 作者主页
    author_url = "https://github.com/JaylenMar"
    # 插件配置项ID前缀
    plugin_config_prefix = "alist2strmpro_"
    # 加载顺序
    plugin_order = 32
    # 可使用的用户级别
    auth_level = 1
    ...
    def init_plugin(self, config: dict = None) -> None:
        if config:
            ...
            self._source_dirs = [d.strip() for d in config.get("source_dirs", "").split("\n") if d.strip()]
    ...
    def alist2strm(self) -> None:
        try:
            ...
            logger.info("Alist2StrmPro 插件开始执行")
            asyncio.run(self.__process())
            logger.info("Alist2StrmPro 插件执行完成")
        except Exception as e:
            logger.error(
                f"Alist2StrmPro 插件执行出错：{str(e)} - {traceback.format_exc()}"
            )
    ...
    def __cached_computed_target_path(self, path: str, suffix: str) -> Path:
        # 查找匹配的源目录（最长路径优先）
        matched_source = ""
        for source in sorted(self._source_dirs, key=lambda x: len(x), reverse=True):
            if path.startswith(source):
                matched_source = source
                break

        if not matched_source:
            return Path(self._target_dir) / path.lstrip("/")

        target_path = Path(self._target_dir) / path.replace(
            matched_source, self._path_replace, 1
        ).lstrip("/")
    ...
    def get_state(self) -> bool:
        return (
            True
            if self._enabled and self._cron and self._token and self._url and self._source_dirs
            else False
        )
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "type": "info",
                                            "variant": "tonal",
                                            "text": "路径替换支持多级目录映射，例：/云盘1/电影 -> /movies"
                                        },
                                    }
                                ],
                            },
